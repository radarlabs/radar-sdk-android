...
object Radar {
        ...
        sendLogConversionRequest(
            ...
            callback = object : RadarLogConversionCallback {
                override fun onComplete(
                    ...
                    this@Radar.logger.i("Conversion type = ${event?.conversionType}: status = $status; event = $event")
                }
            }
        )
     ...
     *
     * @param[conversionType] The type of the conversion.
     * @param[metadata] The metadata associated with the conversion.
     * @param[callback] A callback.
     */
    @JvmStatic
    fun logConversion(conversionType: String, metadata: JSONObject? = null, callback: RadarLogConversionCallback) {
        if (!initialized) {
            callback.onComplete(RadarStatus.ERROR_PUBLISHABLE_KEY)
        ...
        if (isLastTrackRecent || RadarSettings.getPermissionsDenied(context)) {
            sendLogConversionRequest(conversionType, metadata, callback = callback)
        ...
        trackOnce(object : RadarTrackCallback {
            override fun onComplete(status: RadarStatus, location: Location?, events: Array<RadarEvent>?, user: RadarUser?) {
                ...
                sendLogConversionRequest(conversionType, metadata, callback)
            }
        })
    }
    ...
    fun logConversion(
        ...
        logConversion(conversionType, metadata, object : RadarLogConversionCallback {
            override fun onComplete(
                status: RadarStatus,
                event: RadarEvent?
            ) {
    ...
    fun logConversion(
        ...
        logConversion(conversionType, nonNullMetadata, callback)
    }
    ...
    fun logConversion(
        ...
        logConversion(conversionType, revenue, metadata, object : RadarLogConversionCallback {
            override fun onComplete(
                status: RadarStatus,
                event: RadarEvent?
            ) {
